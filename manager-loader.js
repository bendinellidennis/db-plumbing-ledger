...
function installPaymentIntegrity(){
  ...
  M.mPut=async(store,val)=>{
    ...
    if(got<requestedTotal-0.005){payments.push({id:M.uid(),kind:'balance',amount:Number((requestedTotal-got).toFixed(2)),date:'',method:'',note:'Saldo registrato da stato Pagato',createdAt:new Date().toISOString(),system:true})}
    ...
    if(eid){const entry=await M.lOne('entries',eid);if(entry)await M.lPut('entries',{...entry,payments:payments.map(p=>({...p})),paidAmount:Number(requestedTotal.toFixed(2)),paid:'paid'})}
    return out;
  };
}
const wait=()=>{if(started)return;if(!(window.DBM?.db&&document.getElementById('jobsV2'))){setTimeout(wait,50);return}started=true;window.__DBM_MAIN_LOADER_READY__=false;window.__DBM_MAIN_LOADER_PROMISE__=(async()=>{await load(files);installPaymentIntegrity();window.__DBM_MAIN_LOADER_READY__=true;window.dispatchEvent(new Event('dbm-main-loader-ready'))})().catch(err=>{console.error('DB Manager main loader',err);throw err})};
wait();
})();
