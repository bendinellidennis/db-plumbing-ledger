...
function ensurePdf(){
 ...
 if(!window.DBM?.__quotePdfV121Ready)ensure('manager-quote-pdf-v121.js?v=121','data-dbm-quote-pdf-v121');
}
function init(M,nav,jobs){
 if(M.__quotesNavReady){ensurePdf();return}M.__quotesNavReady=true;
 ...
 M.show=v=>{baseShow(v);if(v==='quotesV2'){nav.querySelectorAll('.nav').forEach(x=>x.classList.toggle('active',x===q));setTimeout(ensurePdf,0)}else q.classList.remove('active')};
